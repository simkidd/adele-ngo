...
export default function AnnouncementsPage() {
  ...
  const save = async () => {
    ...
    setError("");
    try {
      if (isNew) await adminApi.post("/announcements", form);
      else await adminApi.patch(`/announcements/${editing!._id}`, form);
      setEditing(null);
      load();
    } catch (err) {
  ...
  const togglePublish = async (a: Announcement) => {
    try {
      await adminApi.patch(`/announcements/${a._id}`, {
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${statusColors[a.status]}`}
                      >
                        {a.status}
                      </span>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      className={`text-xs font-bold px-3 py-1.5 rounded-lg transition-colors ${a.status === "Published" ? "bg-slate-100 hover:bg-slate-200 text-slate-600" : "bg-green-100 hover:bg-green-200 text-green-700"}`}
                    >
                      {a.status === "Published" ? "Unpublish" : "Publish"}
                    </button>
                    <button
                      onClick={() => openEdit(a)}
                      className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-orange-100 hover:text-orange-600 flex items-center justify-center transition-colors"
                    >
                      <Pencil size={14} />
                    </button>
                    <button
            ...
            >
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className={inp}
                    >
                      {["General", "Cohort", "Program", "Alert"].map((t) => (
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className={inp}
                    >
                      {["Public", "Applicants", "Enrolled", "All"].map((a) => (
              ...
              <div className="flex items-center justify-end gap-3 px-7 py-4 border-t border-slate-100">
                ...
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl bg-orange-500 disabled:opacity-40 hover:bg-orange-600 text-white font-bold text-sm flex items-center gap-2 transition-colors"
                >
                  {saving ? (
                    <>
