...
function QRPlaceholder({ value }: { value: string }) {
  ...
  return (
    <div
      className="inline-grid bg-white p-2 rounded-lg border border-slate-200"
      style={{ gridTemplateColumns: `repeat(${size},1fr)`, gap: 1.5 }}
    >
      {cells.map((dark, i) => (
        <div
          key={i}
          style={{
            width: 6,
            height: 6,
            background: dark ? "#1e293b" : "transparent",
            borderRadius: 1,
          }}
        />
      ))}
...
function CertPreview({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 bg-black/70 supports-backdrop-filter:backdrop-blur-xs flex items-center justify-center p-4"
      onClick={onClose}
    >
      <motion.div
      ...
      >
        ...
        <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-10">
          ...
          <p className="font-heading text-4xl font-black text-orange-400 text-center mb-2">
            {cert.graduateName}
          </p>
          <p className="text-slate-400 text-center text-sm mb-1">
            has successfully completed the
          </p>
          <p className="font-heading text-2xl font-black text-white text-center mb-1">
            {cert.programTitle}
          </p>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100">
          <p className="text-sm text-slate-500">
            Verify at:{" "}
            <span className="font-mono text-orange-500 text-xs">
              {verifyUrl}
            </span>
          </p>
...
export default function CertificationsPage() {
  ...
  const enrolledRegs = SEED_REGISTRATIONS.filter(
    (r) =>
      r.status === "Enrolled" &&
      !certs.find(
        (c) => c.graduateName === r.name && c.programTitle === r.programTitle,
      ),
  );
  ...
  const issueCert = () => {
    ...
    const cert: Certificate = {
      id: `cert-${Date.now()}`,
      certId,
      graduateName: reg.name,
      programTitle: reg.programTitle,
      programId: reg.programId,
      cohortName: reg.cohortId ?? "Independent",
  ...
  return (
    <div className="max-w-6xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search name, cert ID..."
            className="pl-9 pr-4 py-2.5 text-sm bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 w-64 transition"
          />
        </div>
        <button
          onClick={() => setShowIssue(true)}
          className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-colors"
        >
              ...
              {filtered.map((cert, i) => (
                ...
                >
                  ...
                  <td className="px-5 py-4">
                    <span className="font-mono text-xs text-orange-600 bg-orange-50 px-2.5 py-1 rounded-lg">
                      {cert.certId}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-semibold bg-slate-100 hover:bg-orange-100 hover:text-orange-600 px-3 py-1.5 rounded-lg transition-colors"
                      >
            ...
            >
              ...
              <div className="px-7 py-6">
                <p className="text-slate-500 text-sm mb-5">
                  Select an enrolled participant to issue a certificate.
                </p>
                {enrolledRegs.length === 0 ? (
                  <div className="text-center py-6">
                    <Award size={32} className="text-slate-300 mx-auto mb-3" />
                    <p className="text-slate-400 text-sm">
                      No enrolled participants awaiting certificates.
                    </p>
                  </div>
                ) : (
      ...
      <AnimatePresence>
        {preview && (
          <CertPreview cert={preview} onClose={() => setPreview(null)} />
        )}
      </AnimatePresence>
    </div>
  );
