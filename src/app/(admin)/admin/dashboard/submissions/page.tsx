...
export default function SubmissionsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const { data } = await adminApiInstance.get("/submissions?limit=50");
      setSubs(data.data);
      setMeta({ total: data.meta?.total ?? 0, unread: data.meta?.unread ?? 0 });
    } catch {}
    setLoading(false);
  }, []);
  useEffect(() => {
  ...
  const markRead = async (id: string) => {
    try {
      await adminApiInstance.patch(`/submissions/${id}/read`);
      setSubs((prev) =>
        prev.map((s) => (s._id === id ? { ...s, read: true } : s)),
      );
      setMeta((m) => ({ ...m, unread: Math.max(0, m.unread - 1) }));
      if (detail?._id === id) setDetail((d) => (d ? { ...d, read: true } : d));
    } catch {}
  };
  const confirmDelete = async () => {
    try {
      await adminApiInstance.delete(`/submissions/${deleteId}`);
      setSubs((prev) => prev.filter((s) => s._id !== deleteId));
      if (detail?._id === deleteId) setDetail(null);
    } catch (err) {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {["", "Name", "Type", "Message", "Date", "Actions"].map(
                    (h, i) => (
                      <th
                        key={i}
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map((sub, i) => (
                  <motion.tr
                    ...
                    className={`hover:bg-slate-50/50 transition-colors cursor-pointer ${!sub.read ? "bg-orange-50/30" : ""}`}
                    onClick={() => {
                      setDetail(sub);
                      if (!sub.read) markRead(sub._id);
                    }}
                  >
                    ...
                    <td className="px-5 py-4">
                      <p
                        className={`font-semibold ${!sub.read ? "text-slate-900" : "text-slate-700"}`}
                      >
                        {sub.name}
                      </p>
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`text-xs font-semibold px-2.5 py-1 rounded-full ${enquiryColors[sub.enquiryType]}`}
                      >
                        {enquiryLabels[sub.enquiryType]}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-slate-500 max-w-xs">
                      <p className="line-clamp-1">{sub.message}</p>
                    </td>
          ...
          >
            ...
            >
              ...
              <div className="p-6 space-y-5">
                <span
                  className={`inline-block text-xs font-semibold px-3 py-1 rounded-full ${enquiryColors[detail.enquiryType]}`}
                >
                  {enquiryLabels[detail.enquiryType]}
                </span>
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-2 w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 rounded-xl text-sm transition-colors"
                >
