...
const roleColors: Record<string, string> = {
  ...
  program_officer: "bg-blue-100 text-blue-700",
  blog_editor: "bg-purple-100 text-purple-700",
};
const inp =
              ...
              <tbody className="divide-y divide-slate-50">
                {users.map((u, i) => (
                  <motion.tr
                    ...
                    className={`hover:bg-slate-50/50 transition-colors ${!u.isActive ? "opacity-50" : ""}`}
                  >
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`text-xs font-bold px-2.5 py-1 rounded-full ${roleColors[u.role]}`}
                      >
                        {u.role.replace("_", " ")}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-slate-600 text-sm">
                      {u.centerId?.name?.replace(" Training Center", "") ?? "—"}
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`text-xs font-semibold ${u.isActive ? "text-green-600" : "text-red-500"}`}
                      >
                        {u.isActive ? "Active" : "Inactive"}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      {canManage && u._id !== currentUser?.id && u.isActive ? (
                        <button
            ...
            >
              ...
              <div className="px-7 py-6 space-y-5">
                ...
                {form.role === "program_officer" && (
                  <div>
                    ...
                    <select
                      value={form.centerId}
                      onChange={(e) => upd("centerId", e.target.value)}
                      className={inp}
                    >
              ...
              <div className="flex items-center justify-end gap-3 px-7 py-4 border-t border-slate-100">
                ...
                <button
                  onClick={save}
                  disabled={
                    saving || !form.fullName || !form.email || !form.password
                  }
